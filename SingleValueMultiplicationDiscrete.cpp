...
void changeIt(vector<vector<double>> &v, int first, int duration) {
    ...
    for(int i = start; i<end; i++) {
        int startTime = (rand() % 48);
        int endTime = (rand() % 9) + 4;
        endTime = startTime + endTime;
        float multi = (rand()%5);
        multi = (multi / 10) + 0.1;
        // cout<<startTime<<" "<<endTime<<" "<<multi<<endl;

        if(endTime > 50) {
            if(i < 170) {
                for(int j = 0; j<endTime - 50; j++)
                    v[i+1][j] *= multi;
            }
...
{
    setIO("");
    freopen("MeterDataForDiscreteGeneration.txt","r",stdin);
    freopen("singleDiscreteMultiplicationTheives.txt","w",stdout);
    srand ( time(NULL) );
    ...
    // cout<<" Data will be from now on : times (6) , then range (1 to 10) mf (0.3 to 0.5) (4 to 12 hr) "<<endl;
